...
def dijkstra_w_pri_queue(W, sv):
    ...
    # Copilot started the original dijkstra function without priority queue
    # ChatGPT-3 added the base case, priority que and a reduced amount of lines.
    ...
    while not pq.empty():
        ...
        for vertex, weight in enumerate(W[curr_vertex]):
            if weight != INF:
                new_distance = distances[curr_vertex] + weight
...
def dijkstra_w_array(W, sv):
    ...
    # Copilot started the original dijkstra_w_array function with little input, chatGPT-3 made it more efficent by adding
    # a complex generator expression for the min_vertex.

    visited, distances = set(), [INF] * len(W)
    distances[sv] = 0

    while len(visited) < len(W):
        min_vertex = min({v for v in range(len(W)) if v not in visited}, key=distances.__getitem__)
        visited.add(min_vertex)
        for vertex in range(len(W)):
            if vertex not in visited and W[min_vertex][vertex] != INF:
                distances[vertex] = min(distances[vertex], distances[min_vertex] + W[min_vertex][vertex])
...
def assign03_main():
    ...
    g = adjMatFromFile("graph_100verts_B.txt")
    if len(g) <= 20:
